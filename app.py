...
def get_documents():
    ...
    result = [{'id': d.id, 'filename': d.filename, 'uploaded_at': d.uploaded_at.isoformat(), 'chunks': len(d.chunks)} for d in docs]
    db.close()
...
def upload_document():
    ...
    for i, chunk in enumerate(chunks):
        chunk_obj = Chunk(id=str(uuid.uuid4()), document_id=doc_id, chunk_index=i, text=chunk)
        db.add(chunk_obj)
...
def chat():
    ...
    user_msg = Message(id=str(uuid.uuid4()), conversation_id=conv_id, role='user', content=question)
    db.add(user_msg)
...
Be concise and helpful."""
    ...
    ai_msg = Message(id=str(uuid.uuid4()), conversation_id=conv_id, role='assistant', content=answer)
    db.add(ai_msg)
...
def get_qa_history():
    ...
    result = [{'id': h.id, 'question': h.question, 'answer': h.answer, 'created_at': str(h.created_at)} for h in history]
    db.close()
    return jsonify(result)
...
def get_chunks():
    ...
    result = [{'id': c.id, 'text': c.text[:200] + '...', 'index': c.chunk_index} for c in chunks]
    db.close()
    return jsonify(result)
